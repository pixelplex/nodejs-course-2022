...
app.post(
  ...
  body('email')
    ...
    .isEmail()
    .custom((value) => {
      const conflict = users.find((user) => user.email === value);
...
app.get(
  '/post/search',
  query('phrase').isString().trim().isLength({ min: 1 }).toLowerCase(),
  validate,
  (req, res, next) => {
    try {
      const filteredPosts = posts.filter(
        (p) => p.content.toLowerCase().includes(req.query.phrase) || p.title.toLowerCase().includes(req.query.phrase)
      );
...
app.get('/post/:postId', param('postId').isInt({ allow_leading_zeroes: false }).toInt(), validate, (req, res, next) => {
  try {
