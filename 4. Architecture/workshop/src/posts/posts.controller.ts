...
export class PostsController {
  ...
  static getOnePost = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const post = await PostsService.findById(req.params.postId as unknown as number);
      if (!post) {
  ...
  static updatePost = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const updatedPost = await PostsService.update(req.params.postId as unknown as number, req.body);
      if (!updatedPost) {
        throw new AppError("Couldn't find post", 404);
      }
      res.status(200).json({
  ...
  static deletePost = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const id = await PostsService.delete(req.params.postId as unknown as number);
      if (id === null) {
        throw new AppError("Couldn't find post", 404);
      }
      res.status(200).json({
